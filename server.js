...
try:
    ...
    result = try_login(cl, '${cleanUser}', '${safePass}')

    if result.get("success"):
        session = json.dumps(cl.get_settings())
        print(json.dumps({"success": True, "userId": str(cl.user_id), "username": "${cleanUser}", "sessionData": session}))
...
`;
    ...
    setTimeout(() => { py.kill(); resolve({ success: false, error: "Login timeout — Instagram not reachable. Try a different proxy." }); }, 60000);
  });
}
...
try:
    ...
    try:
        ...
        err = str(e2)
        if "challenge" in err.lower() or "verify" in err.lower() or "feedback" in err.lower(): print(json.dumps({"pending": True}))
        else: print(json.dumps({"success": False, "error": err}))
except Exception as e:
    print(json.dumps({"success": False, "error": str(e)}))
`;
    const py = spawn("python3", ["-c", script]);
